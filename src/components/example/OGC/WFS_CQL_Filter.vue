...
export default {
    ...
    methods: {
        _addWFS_CQL_Filter() {
            ...
            const CQL_FILTER = "NAME LIKE '%公园%'";
            const CQL_FILTER_URL = encodeURI(CQL_FILTER);

            const vectorLayer = new VectorLayer({
                source: new VectorSource({
                    format: new GeoJSON(),
                    url: `${baseUrl}?service=${service}&version=${version}&request=${request}&typeName=${typeName}&outputFormat=${outputFormat}&CQL_FILTER=${CQL_FILTER_URL}`,
                }),
                style: new Style({
                    image: new Icon({
