    ...
    addKrigingLayer() {
      ...
      for (let i = 0; i < n; i++) {
        ...
        var featurePoint = new Feature({
          geometry: new Point(olProj.transform([x[i], y[i]], "EPSG:4326", "EPSG:3857")),
          value: t[i],
        });
      ...
      for (var i = 0; i < polygonfeatures.length; i++) {
        //坐标转换
        polygonfeatures[i].getGeometry().transform("EPSG:4326", "EPSG:3857");
      }
      ...
      //圈定范围不管是坐标系4326还是3857，必须传入的的经纬度4326
      //let extent = polygonLayer.getSource().getExtent(); //圈定范围
      //console.log(extent);
      let extent = [116.799027, 40.35813498, 116.8598099, 40.388326];
      if (t.length > 3) {
        //样本训练
        ...
        this.canvasLayer = new ImageLayer({
          source: new ImageCanvasSource({
            ...
            projection: "EPSG:4326",//这里必须是4326
          }),
        });
